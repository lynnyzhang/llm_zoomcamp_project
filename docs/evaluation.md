...
Generated by `evaluation/data/src/generate_qa.py` into `evaluation/data/qa.jsonl`,
...
- At eval time the ground-truth answer is the linked document's own content
  ...
  "answer" lookup (`answer_orig = doc_idx[doc_id]["answer"]`).
   Shared helpers live in `evaluation/data/src/generate_qa.py` (`map_progress`,
   a tqdm wrapper for parallel eval work).

## 1. Retrieval Evaluation

**Notebook:** `evaluation/notebooks/04_retrieval_quality.ipynb`
**Output:** `evaluation/notebooks/results/retrieval_eval.json`
...
**Output:** `evaluation/notebooks/results/llm_eval.json`
...
- **Relevance (4.4-4.7) and coherence (4.8-4.9) are high**; **faithfulness is the weak dimension (3.0-3.9)**. Generated answers add details (stats, evolution levels) beyond the single retrieved document, which the judge counts against context support. This is a known gap.
...
**Output:** `evaluation/notebooks/results/agent_eval.json`, `evaluation/notebooks/results/agent_eval_comparison.png`
...
#      - 04_retrieval_quality.ipynb   → evaluation/notebooks/results/retrieval_eval.json (no LLM)
#      - 05_answer_quality.ipynb      → evaluation/notebooks/results/llm_eval.json (needs LLM; ~19 min on dev subset)
#      - 01_agent_path_analysis.ipynb → evaluation/notebooks/results/agent_eval.json (needs LLM; ~27 min on dev subset)
```
