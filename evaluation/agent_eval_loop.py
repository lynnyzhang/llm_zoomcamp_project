...
def evaluate_agent_loop(agent, rag_base, questions, llm_available, show_avg=False):
    search_counts = []
    hits = 0
    latencies = []
    ...
    for i, pair in enumerate(questions):
        ...
        if (i + 1) % 10 == 0:
            if show_avg:
                print(
                    f"  [{i + 1}/{len(questions)}] processed, avg searches: {np.mean(search_counts):.2f}"
                )
            else:
...
def judge_agent_answers(agent: RAGAgent, client, questions: list[dict], doc_idx: dict, llm_available: bool, judge_sample: int):
    ...
    for i, pair in enumerate(questions):
        ...
        if ground_truth is None:
            print(
                f"  [{i + 1}/{len(questions)}] No ground-truth document for question {pair.get('document')}"
            )
            agent_errors += 1
            continue
        ...
        judge_result["source"] = result.source
        judge_result["confidence"] = result.confidence
    ...
    agent_mean_score = (
        round(np.mean([s["score"] for s in agent_quality_scores]), 2)
